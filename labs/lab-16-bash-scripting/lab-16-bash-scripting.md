...
## Ready-to-run scripts (assets)

Two complete, commented scripts are in [`scripts/`](scripts/) — study them for variables, loops, conditionals, functions and traps, then run them on the **Killercoda Ubuntu** playground:

- [`scripts/server-healthcheck.sh`](scripts/server-healthcheck.sh) — a threshold-based disk / load / service health check.
  ```bash
  bash scripts/server-healthcheck.sh
  ```
- [`scripts/backup-rotate.sh`](scripts/backup-rotate.sh) — a daily tar backup with retention (pairs with Lab 26).
  ```bash
  bash scripts/backup-rotate.sh /etc /root/backups 7
  ```
