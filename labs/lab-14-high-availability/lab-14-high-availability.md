...
Kubernetes provides HA the same way HAProxy + keepalived do here, but at the orchestration layer: multiple replicas behind one load-balanced Service, with self-healing. Apply [`assets/k8s-ha-deployment.yaml`](assets/k8s-ha-deployment.yaml) on the **Killercoda Kubernetes** playground (https://killercoda.com/playgrounds/scenario/kubernetes):
