...
## Containerised roles (bonus assets)

The same Web / File / Database roles run as containers. Build them with **Docker Desktop** (https://www.docker.com/products/docker-desktop/) using the files in [`assets/`](assets/):

- [`assets/Dockerfile`](assets/Dockerfile) — the web role as an nginx image.
- [`assets/docker-compose.yml`](assets/docker-compose.yml) — the full Web + File + Database stack:
  ```bash
  docker compose up -d      # start all three roles
  docker compose ps
